...
/**
  ...
  @param FdtBlob The device tree. Is extended to fit the hart id.
  ...
  )
{
  ...
  Size = fdt_totalsize(FdtBlob);
  Err  = fdt_open_into(FdtBlob, FdtBlob, Size + 32);
  if (Err < 0) {
    DEBUG ((DEBUG_ERROR,
      "Device Tree can't be expanded to accommodate new node\n", __FUNCTION__));
    return EFI_OUT_OF_RESOURCES;
  }
  ChosenOffset = fdt_path_offset(FdtBlob, "/chosen");
  fdt_setprop_u32(FdtBlob, ChosenOffset, "boot-hartid", BootingHartId);
...
InstallFdtFromHob (VOID)
{
  ...
  GuidHob = GetFirstGuidHob (&gRiscVFlattenedDeviceTreeHobGuid);
  if (GuidHob == NULL) {
    DEBUG ((DEBUG_ERROR, "Failed to find RISC-V FDT Hob\n",
      __FUNCTION__));
  ...
  Status = FixFdt (DataInHob, PcdGet32(PcdBootHartId));
  if (EFI_ERROR (Status)) {
    return Status;
  }
